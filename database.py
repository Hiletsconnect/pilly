...
def get_db():
    """
    FastAPI dependency.
    Devuelve un CURSOR (DictCursor) para que tus routers puedan hacer:
      db.execute(...); db.fetchone(); db.fetchall()
    """
    conn = _connect()
    cur = conn.cursor()
    try:
...
def init_db():
    ...
    try:
        ...
        # --- seed admin ---
        admin_user = (settings.DEFAULT_ADMIN_USER or "").strip()
        ...
        if admin_user and admin_pass:
            ...
            print("[db][WARN] DEFAULT_ADMIN_USER o DEFAULT_ADMIN_PASS vacío -> no se crea admin automáticamente.")
